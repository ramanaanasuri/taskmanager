...
if have_cmd docker; then
  ...
  echo "❌ Neither Docker nor npx is available."
...
if [[ -z "${PUB:-}" || -z "${PRV:-}" ]]; then
  echo "❌ Could not parse public/private keys from:"
  echo "$RAW_JSON"
  exit 1
fi
...
echo "📝 Wrote backend env: $OUT_ENV_BACKEND"
...
🔒 Keep the PRIVATE key secret (Secrets Manager or environment only). Do not commit it to a public repo.
