...
public class TaskAgentTools {
    ...
    private static String line(Task t) {
        return "{id:" + t.getId()
             + ", title:\"" + t.getTitle() + "\""
             + ", priority:" + t.getPriority()
             + ", due:" + (t.getDueDate() == null ? "none" : t.getDueDate().format(ISO))
    ...
    public static class ListTasks implements AgentTool {
        ...
        public String execute(String userEmail, JsonNode args) {
            ...
            tasks.forEach(t -> sb.append(line(t)).append('\n'));
            return sb.toString();
        }
    }
    ...
    public static class CreateTask implements AgentTool {
        ...
        public String execute(String userEmail, JsonNode args) {
            ...
            if (due != null && !due.isBlank()) {
                try { t.setDueDate(LocalDateTime.parse(due, ISO)); }
    ...
    public static class UpdateTask implements AgentTool {
        ...
        public String execute(String userEmail, JsonNode args) {
            ...
            if (args.hasNonNull("dueDate")) {
                try { t.setDueDate(LocalDateTime.parse(args.get("dueDate").asText(), ISO)); }
                catch (Exception e) { return "ERROR: dueDate must be YYYY-MM-DDTHH:mm."; }
            }
            t.setUpdatedAt(LocalDateTime.now());
            return "Updated: " + line(repo.save(t));
        }
    }
    ...
    public static class CompleteTask implements AgentTool {
        ...
        public String execute(String userEmail, JsonNode args) {
            ...
            t.setCompleted(true);
            t.setUpdatedAt(LocalDateTime.now());
            return "Completed: " + line(repo.save(t));
        }
    }
    ...
    public static class ScheduleNotification implements AgentTool {
        ...
        public String execute(String userEmail, JsonNode args) {
            ...
            t.setUpdatedAt(LocalDateTime.now());
            return "Notification updated: " + line(repo.save(t));
        }
    }
}
